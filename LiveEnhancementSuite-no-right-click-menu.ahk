...
Hotkey, !r, returntracks
Hotkey, !d, delays
Hotkey, +!c, colortracks
Hotkey, !c, colortracks12
...
MouseGetPos,,,guideUnderCursor
WinGetTitle, WinTitle, ahk_id %guideUnderCursor%
if(InStr(WinTitle, "Ableton Live 12") != 0){
	WinMenuSelectItem, , , View, Arrangement Track Controls, Return Tracks
}
Return
...
WinGetTitle, WinTitle, ahk_id %guideUnderCursor%
if(InStr(WinTitle, "Ableton Live 12") != 0){
	WinMenuSelectItem, , , View, Arrangement Track Controls, Track Options
}
Return
...
coolfunc:
	...
	Loop %windows% {
		...
		if (RegExMatch(wt, "- Ableton Live 12") != 0){
			mainwindowtitle := wt
			if (RegExMatch(wt, "unsaved") = 0) && (RegExMatch(wt, "\*") != 0) {
				trackname := RegExReplace(wt, "\*\s\-\s([^\]]+)$", "")
				; Checks if a file is saved with versioning (ex: _2 or _3 at the end) and removes it
			    if (RegExMatch(trackname, "_(\d+)$") != 0) {
			        ; Remove underscore and number at the end
			        trackname := RegExReplace(trackname, "_(\d+)$", "")
			    }
				trackname := RegExReplace(trackname, "[\W_]+", "_")
			}
			else if (RegExMatch(wt, "unsaved") = 0) {
				trackname := RegExReplace(wt, "\s\-\sAbleton([^\]]+)$", "")
				; Checks if a file is saved with versioning (ex: _2 or _3 at the end) and removes it
			    if (RegExMatch(trackname, "_(\d+)$") != 0) {
			        ; Remove underscore and number at the end
			        trackname := RegExReplace(trackname, "_(\d+)$", "")
		...
		else if (RegExMatch(wt, "- Ableton Live") != 0){
			mainwindowtitle := wt
